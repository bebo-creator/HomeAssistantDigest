...
/**
 * NEW: Extract and analyze Versatile Thermostat (TPI) data from snapshots
 * This enables full analysis of climate entities with VT algorithm
 */
function extractTPIAnalysis(snapshots) {
    const tpiData = {};
    
    for (const snap of snapshots) {
        if (snap.entity_id && snap.entity_id.startsWith('climate.')) {
            if (!tpiData[snap.entity_id]) {
                tpiData[snap.entity_id] = {
            ...
            tpiData[snap.entity_id].states.push(snap.value_str || snap.value_num);
            
            if (snap.attributes) {
                const attrs = snap.attributes;
                
                // TPI Coefficients
                if (attrs.tpi_coef_int !== undefined || attrs.tpi_coef_ext !== undefined) {
                    tpiData[snap.entity_id].tpi_samples.push({
                        timestamp: snap.timestamp,
                        coef_int: attrs.tpi_coef_int,
                        coef_ext: attrs.tpi_coef_ext,
                        threshold_low: attrs.tpi_threshold_low,
                        threshold_high: attrs.tpi_threshold_high
                    });
                }
                
                // Power and activation metrics
                if (attrs.on_percent !== undefined || attrs.power_percent !== undefined) {
                    tpiData[snap.entity_id].power_samples.push({
                        timestamp: snap.timestamp,
                        on_percent: attrs.on_percent,
                        power_percent: attrs.power_percent,
                        on_time_sec: attrs.on_time_sec,
                        off_time_sec: attrs.off_time_sec,
                        function: attrs.function
                    });
                }
                
                // Temperature algorithm data
                if (attrs.current_temperature !== undefined || attrs.ema_temp !== undefined) {
                    tpiData[snap.entity_id].temp_data.push({
                        timestamp: snap.timestamp,
                        current_temp: attrs.current_temperature,
                        target_temp: snap.attributes.target_temperature,
                        ext_temp: attrs.ext_current_temperature,
                        temp_slope: attrs.temperature_slope,
                        ema_temp: attrs.ema_temp,
                        total_energy: attrs.total_energy
                    });
                }
            }
        }
...
/**
 * NEW: Format TPI analysis section for Gemini prompt
 */
function formatTPISection(tpiData) {
    if (!tpiData || Object.keys(tpiData).length === 0) return '';
    
    const lines = ['## Versatile Thermostat (TPI) Algorithm Analysis'];
    
    for (const [entityId, data] of Object.entries(tpiData)) {
        lines.push(`\n### ${data.friendly_name}`);
        
        if (data.states.length > 0) {
            const state = data.states[data.states.length - 1];
            lines.push(`**State**: ${state}`);
        }
        
        if (data.tpi_samples.length > 0) {
            const latest = data.tpi_samples[data.tpi_samples.length - 1];
            const first = data.tpi_samples[0];
            
            lines.push(`**TPI Configuration**:`);
            if (latest.coef_int !== undefined) lines.push(`  • Int Coefficient: ${latest.coef_int} (was ${first.coef_int})`);
            if (latest.coef_ext !== undefined) lines.push(`  • Ext Coefficient: ${latest.coef_ext} (was ${first.coef_ext})`);
            if (latest.threshold_low !== undefined) lines.push(`  • Low Threshold: ${latest.threshold_low}`);
            if (latest.threshold_high !== undefined) lines.push(`  • High Threshold: ${latest.threshold_high}`);
        }
        
        if (data.power_samples.length > 0) {
            const latest = data.power_samples[data.power_samples.length - 1];
            const avgPower = data.power_samples.reduce((s, p) => s + (p.power_percent || 0), 0) / data.power_samples.length;
            
            lines.push(`**Power & Activation**:`);
            if (latest.on_percent !== undefined) lines.push(`  • Current On%: ${latest.on_percent}%`);
            if (latest.power_percent !== undefined) lines.push(`  • Current Power: ${latest.power_percent}%`);
            lines.push(`  • Average Duty Cycle: ${avgPower.toFixed(1)}%`);
            
            if (latest.on_time_sec && latest.off_time_sec) {
                const onMin = (latest.on_time_sec / 60).toFixed(1);
                const offMin = (latest.off_time_sec / 60).toFixed(1);
                lines.push(`  • Cycle: ${onMin}m ON, ${offMin}m OFF`);
            }
        }
        
        if (data.temp_data.length > 0) {
            const latest = data.temp_data[data.temp_data.length - 1];
            
            lines.push(`**Temperature Control**:`);
            if (latest.current_temp !== undefined) lines.push(`  • Current: ${latest.current_temp}°C`);
            if (latest.target_temp !== undefined) lines.push(`  • Target: ${latest.target_temp}°C`);
            if (latest.current_temp !== undefined && latest.target_temp !== undefined) {
                lines.push(`  • Overshoot: ${(latest.current_temp - latest.target_temp).toFixed(2)}°C`);
            }
            
            if (latest.ext_temp) lines.push(`  • External Sensor: ${latest.ext_temp}°C`);
            if (latest.ema_temp) lines.push(`  • EMA Prediction: ${latest.ema_temp}°C`);
            if (latest.temp_slope) lines.push(`  • Temperature Slope: ${latest.temp_slope.toFixed(3)}°C/hour`);
            if (latest.total_energy) {
                const kwh = (latest.total_energy / 3600000).toFixed(2);
                lines.push(`  • Total Energy: ${kwh} kWh`);
            }
        }
        
        // Performance Assessment
        if (data.temp_data.length > 3) {
            const temps = data.temp_data.map(t => t.current_temp).filter(t => t !== undefined);
            const targets = data.temp_data.map(t => t.target_temp).filter(t => t !== undefined);
            
            if (temps.length > 0 && targets.length > 0) {
                const minTemp = Math.min(...temps);
                const maxTemp = Math.max(...temps);
                const avgTarget = targets.reduce((a, b) => a + b, 0) / targets.length;
                const tempRange = maxTemp - minTemp;
                const avgOvershoot = temps.map((t, i) => (t - (targets[i] || avgTarget))).reduce((a, b) => a + b, 0) / temps.length;
                
                lines.push(`**Performance**:`);
                lines.push(`  • Temperature Range: ${tempRange.toFixed(2)}°C`);
                lines.push(`  • Average Overshoot: ${avgOvershoot.toFixed(2)}°C`);
                
                if (tempRange > 1.0) {
                    lines.push(`  ⚠️ **Issue**: High temperature oscillation detected`);
                } else if (tempRange < 0.1) {
                    lines.push(`  ✓ Excellent stability`);
                } else {
                    lines.push(`  ✓ Good control`);
                }
            }
        }
    }
...
function buildAnalysisPrompt(profile, entities, entityStats, snapshots, type, addonReport = null, automationReport = null, integrationReport = null, batteryPredictions = [], dismissedWarnings = [], logReport = null, updateReport = null, failedAutomations = null, userNotes = null, previousDigest = null) {
    ...
    if (addonReport && addonReport.total > 0) {
        ...
        const addonSummary = [
            ...
            ...addonReport.issues.filter(i => !i.issue.includes('auto-start')).map(i => `- ⚠️ ${i.addon}: ${i.issue}`)
        ].filter(Boolean);
...
`;
    ...
    // NEW: Extract and format TPI analysis
    const tpiData = extractTPIAnalysis(snapshots);
    const tpiSection = formatTPISection(tpiData);
...
${addonSection}${automationSection}${integrationSection}${batterySection}${logSection}${updateSection}${failedAutoSection}${dataQualitySection}${tpiSection}
${previousContext}
...
### TPI Analysis (Versatile Thermostat)
- If TPI data is available, analyze temperature stability and oscillation patterns
- Flag high temperature overshoot (>1°C) as a warning issue
- Suggest TPI coefficient adjustments if oscillation is detected
- Consider energy consumption patterns in context of duty cycle
